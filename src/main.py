...
def find_largest_interior_rectangle(binary_image, contour):
...
class VideoProcessor:
    ...
    def process_frame(self, frame):
        ...
        try:
            top_left, bottom_right = find_largest_interior_rectangle(binary, contour)
            ...
            if (width_cm > threshold) and (height_cm > threshold):
                reusability = True

            text = "Reusable" if reusability else "Not Reusable"
            print("[+] Width:", width_cm, "cm", "Height:", height_cm, "cm", text)

            color = (255, 0, 0) if reusability else (0, 0, 255)
            cv2.rectangle(frame, top_left, bottom_right, color, 2)

            font_color = (0, 255, 0) if reusability else (0, 0, 255)
